...
class VehicleDetectionPipeline:
    def __init__(self, car_min_confidence=0.4, enable_database=True, enable_events=True):
        ...
        # Stats temporales para videos (en memoria, no persisten)
        self.video_stats = {
            'entries': 0,
            'exits': 0,
            'current_inside': set(),  # Set de track_ids dentro
            'last_entry': None  # {'track_id': X, 'timestamp': datetime, 'plate': 'XXX'}
        }
    ...
    def reset(self):
        ...
        Limpia tracker, cache de vehiculos conocidos, eventos y stats temporales.
        """
        ...
        print("[PIPELINE-RESET] Stats temporales reseteadas")
            ...
            if self.enable_events and self.event_detector:
                try:
                    ...
                    for event in events:
                        ...
                        # Actualizar stats temporales (siempre, con o sin BD)
                        if event['event'] == 'entry':
                            self.video_stats['entries'] += 1
                            self.video_stats['current_inside'].add(track_id)
                            self.video_stats['last_entry'] = {
                                'track_id': track_id,
                                'timestamp': event['timestamp'],
                                'plate': plate
                            }
                            print(f"[VIDEO-STATS] Entrada registrada - Total dentro: {len(self.video_stats['current_inside'])}")
                        
                        elif event['event'] == 'exit':
                            self.video_stats['exits'] += 1
                            self.video_stats['current_inside'].discard(track_id)
                            print(f"[VIDEO-STATS] Salida registrada - Total dentro: {len(self.video_stats['current_inside'])}")
                        
                        # Procesar en BD solo si está habilitada
    ...
    def get_video_stats(self):
        """
        Obtiene estadisticas temporales del video actual (en memoria).
        ...
        """
        return {
            'inside': len(self.video_stats['current_inside']),
            'entries': self.video_stats['entries'],
            'exits': self.video_stats['exits'],
            'last_entry': self.video_stats['last_entry']
        }
